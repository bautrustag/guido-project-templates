# [NOME PROGETTO] — Istruzioni Kimi (via Aider)

> Questo file viene letto automaticamente da Aider all'avvio.
> Kimi K2.5 ha context window di 262k tokens, ma tieni comunque questo file focalizzato.
> Limite consigliato: < 150 righe. Dettagli → docs/
...
Questo progetto viene sviluppato con **più AI coding assistants** (Claude Code, Codex, Gemini, Kimi).
...
- `docs/STORICO-SESSIONI.md` → Aggiungi riga: `| DATA | Cosa fatto | File | Kimi (Aider) | Note |`
...
- `npm run build` deve passare dopo OGNI modifica — verificalo con /run npm run build
...
### `/clear-safe` — Salva stato prima di chiudere
1. Aggiorna `docs/SESSION-STATE.md` con stato attuale, prossimi passi, stato build
2. Aggiungi riga a `docs/STORICO-SESSIONI.md`: `| DATA | Lavoro | File | Kimi (Aider) | Note |`
3. Se hai risolto un bug non ovvio → aggiungi entry a `docs/REMINDERS.md`
4. Conferma: "✅ Stato salvato nei file condivisi. Puoi chiudere."

### `/status` — Mostra stato progetto
1. Leggi e mostra contenuto di `docs/SESSION-STATE.md`
2. Esegui `/run npm run build` e mostra risultato
...
---
_Ultimo update: [DATA] | Righe: ~120/150 | Tool: Kimi K2.5 (Aider)_
