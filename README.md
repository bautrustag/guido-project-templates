...
| `/@testing` | Test automatici, Playwright, audit |

---

## 🧪 Testing Automatico

La cartella `testing/` contiene script per test automatici:

### Setup nel progetto

```bash
# Copia file testing nel progetto
...
npm run test:e2e:ui     # UI debug interattiva
```

### Personalizzazione

1. `scripts/audit.mjs` → Aggiungi le tue PAGES
2. `e2e/tests/pagine.spec.ts` → Aggiungi le tue PAGES
3. `e2e/tests/critical-path.spec.ts` → I tuoi flussi business

Vedi `testing/README.md` e `docs-templates/TESTING.md` per la guida completa.
